...
module.exports = function Databattle() {
  ...
  for (var y = 0; y < 13; y++) {
    for (var x = 0; x < 16; x++) {
      this.cells[y][x] = new Cell(true, y*16 + x % 9);
    }
  }
  ...
  this.performAction = function(action, loc) {
    var target = that.cells[loc.x][loc.y].occupier;
    switch (action.type) {
    case 'damage':
      if (!occupier) throw new Error("Invalid attack target.");
      else
        var res = occupier.damage(action.magnitude);
  ...
  function doNextMove() {
    ...
    whoeversTurnItIs.getMove(function(action) {
      ...
      case 'step':
        ...
        action.unit.step(action.target, grid);
        return;
      case 'attack':
